...
y_labelled = labelled_df['root_cau']

root_caus = ['Access Validation Error', 'Boundary Condition Error', 'Configuration Error', 'Input Validation Error',
            'Failure to Handle Exception', 'Race Condition Error']
...
X_train_labelled, X_test_labelled, y_train_labelled, y_test_labelled = train_test_split(
    X_labelled, y_labelled_codes, test_size=0.2, stratify=y_labelled, random_state=19
)
...
X_unlabelled_1, X_unlabelled_2 = train_test_split(
    X_unlabelled, test_size=0.01, random_state=3
)
...
lp_root_cau = LabelPropagation(max_iter=500, kernel="rbf", n_neighbors=4, gamma=100)
lp_root_cau.fit(X_total_trans, y_train_labelled_con)


# 基于相似度去噪
# 将数据分到不同的桶中
n_buckets = int(len(X_total_trans)/3)
bucket_indices = KMeans(n_clusters=n_buckets, random_state=8).fit_predict(X_total_trans)
...
for bucket_idx in range(n_buckets):
    ...
    cluster_counts = {}
    for i in indices:
        cluster = lp_root_cau.predict([X_total_trans[i]])[0]
    ...
    for i in indices:
        if lp_root_cau.predict([X_total_trans[i]])[0] not in top_types:
            lp_root_cau.transduction_[i] = -1

# 找到并删除噪声样本
noise_idx = np.where(lp_root_cau.transduction_ == -1)[0]
...
# 重新训练标签传播模型
lp_root_cau = LabelPropagation(max_iter=100, kernel="rbf", n_neighbors=4, gamma=100)
lp_root_cau.fit(X_total_trans_cleaned, y_train_labelled_cleaned)
...
for i, indices in enumerate(most_similar_indices):
    ...
    label_counts = np.bincount(lp_root_cau.transduction_[similar_indices])
    if len(label_counts) == 0 :
        drop_indices.append(i+len(X_total_trans))
        most_probable_label = -1
    else:
        most_probable_label = np.argmax(label_counts)
    if len(X_total_trans) + i >= len(lp_root_cau.transduction_):
        lp_root_cau.transduction_ = np.concatenate(
            (lp_root_cau.transduction_, [-1 for i in range(len(X_unlabelled_1))]))
    lp_root_cau.transduction_[len(X_total_trans) + i] = most_probable_label

# 从特征矩阵中删除
X_unlabelled_1_trans = np.delete(X_unlabelled_1_trans, drop_indices, axis=0)
lp_root_cau.transduction_ = np.delete(lp_root_cau.transduction_, drop_indices)

y_test_pred = lp_root_cau.predict(X_test_labelled_trans)
